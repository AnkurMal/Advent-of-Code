...
pub fn part_1() {
    ...
    loop {
        if (x>0 && x<(lines[0].len()-1) as i32) && (y>0 && y<(lines.len()-1) as i32) {
            let next = lines[(y+dy) as usize].chars().nth((x+dx) as usize).unwrap();
...
pub fn part_2() {
    let mut grid = DATA.lines().map(|x| x.chars().collect()).collect::<Vec<Vec<char>>>();
    let row = grid.len();
    let col = grid[0].len();
    let mut count = 0;
    ...
    for i in 0..row {
        for j in 0..col {
            ...
            loop {
                if ((x+dx)>=0 && (x+dx)<col as i32) && ((y+dy)>=0 && (y+dy)<row as i32) {
