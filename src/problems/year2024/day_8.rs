                ...
                for(ind_j, &j) in line.iter().enumerate().skip(ind_i+1) {
                    for (ind_ch_j, ch_j) in j.chars().enumerate() {
                        if ch_i==ch_j {
                            ...
                            if !coord.contains(&up_coord) && up_coord[1] >= 0 && up_coord[0]<st_len && up_coord[0]>=0 {
                                coord.push(up_coord);
                            }
                            if !coord.contains(&down_coord) && down_coord[1]<line_len && down_coord[0]<st_len && down_coord[0]>=0 {
                    ...
                    for (ind_ch_j, ch_j) in j.chars().enumerate() {
                        if ch_i==ch_j {
                            ...
                            loop {
                                ...
                                if down_coord[1]<line_len && down_coord[0]<st_len && down_coord[0]>=0 {
                                    down_check = true;
