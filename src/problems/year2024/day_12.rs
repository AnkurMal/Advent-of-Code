            ...
            if let Search::NotDone(ch) = grid[i][j] {
                ...
                while let Some((y, x)) = rec.pop() {
                    ...
                    if y != 0 {
                        ...
                        if curr_ch == Search::NotDone(ch) {
                            if !rec.contains(&new_coord) {
                                rec.push(new_coord);
                            }
            ...
            if let Search::NotDone(ch) = grid[i][j] {
                ...
                while let Some((y, x)) = rec.pop() {
                    ...
                    if ((y < height - 1 && x < width - 1)
                        && (Search::Done(ch) == grid[y][x + 1]
                            || Search::NotDone(ch) == grid[y][x + 1])
                        && (Search::Done(ch) == grid[y + 1][x]
                            || Search::NotDone(ch) == grid[y + 1][x])
                        && (y == 0
                            || (Search::Done(ch) != grid[y - 1][x]
                                && Search::NotDone(ch) != grid[y - 1][x]))
                        && (x == 0
                    ...
                    if ((y < height - 1 && x < width - 1)
                        ...
                        && (Search::Done(ch) == grid[y + 1][x]
                            || Search::NotDone(ch) == grid[y + 1][x])
                        && (Search::Done(ch) != grid[y + 1][x + 1]
                            && Search::NotDone(ch) != grid[y + 1][x + 1]))
                    {
                        corners += 1;
                    }
                    ...
                    if y != 0 {
                        let curr_ch = grid[y - 1][x];
                        let new_coord = (y - 1, x);
                        if curr_ch == Search::NotDone(ch) && !rec.contains(&new_coord) {
                            rec.push(new_coord);
                        }
                    }
